...
/**POST METHODS */
router.route('/register').post(Auth,verifyRole('admin'),controller.register); //register user
router.route('/authentication').post((req,res)=>res.end());  // authenticate user
router.route('/login').post(controller.login);  // login in app
router.route('/sendEmail').post(Auth,verifyRole('admin'),notification.sendEmail); 
...
/**PUT METHODS */
router.route('/updateUser').put(Auth,verifyRole('admin'),controller.updateUser);   //update user profile
